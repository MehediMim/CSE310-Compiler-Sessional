...
private:
    ...
    // unsigned int hashFunc ( string str , unsigned int num_buckets ) {
        ...
        //     for ( unsigned int i = 0; i < len ; i ++){
            ...
            //     return hash ;
            // }
    function<unsigned int(string, unsigned int)> hashFunc = getHashFunctionByName("SDBM");
            
            
    public:
            
    string selectHash;
    ScopeTable(int num_buckets,ScopeTable* parent=nullptr,string HF){
    ...
    ~ScopeTable() {
        for (int i = 0; i < num_buckets; ++i) {
            SymbolInfo* current = symbolinfoList[i];
            while (current != nullptr) {
                SymbolInfo* next = current->getNext(); // Save next before deleting
                delete current;                        // Delete current node
                current = next;                        // Move to next
            }
        }
        delete[] symbolinfoList; // Free the array of pointers
        symbolinfoList = nullptr;
